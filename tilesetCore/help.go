...
func printHelp() {
	fmt.Printf(
		"tilesetCore.exe %s=<input> %s=<output> [%s=<cpu|gpu>] [%s=<true|false>] [%s=<true|false>]\n",
		INPUT, OUTPUT, CORE, MODIFY_HUI, XP_RESIZE)
...
func printHelpInDetail() {
	...
	fmt.Println(XP_RESIZE, ": whether resize the img to 768x768")
}
